...
## function.
##
##  Copyright (C) 2021 Ronald van Engelen <r.v.engelen+alsacapabilities@gmail.com> 
...
has parent cmd=\`${parent_cmd}' with ownerpid=\`${parent_pid}'."
	...
	while read -d $'\0' line; do
        ...
        if [[ "${locking_cmdline[0]}" =~ pulseaudio ]]; then
            printf -v msg_inuse "%s\n%s" "${MSG_ERROR_PULSEAUDIO}" "${MSG_ERROR_PASUSPENDER}"
        else
            printf -v msg_cmdline "%s " "${locking_cmdline[@]}"
            msg_cmdline="${msg_cmdline% *}"
            printf  -v msg_inuse "(%s \`%s' with PID %s)" \
               "${MSG_ERROR_INUSE}" \
	       "${msg_inuse}" \
               "${locking_pid}" 
            printf -v msg_inuse "%s " "${locking_cmdline[@]}"            
        fi
        printf "${c_std}${c_dim}(%s).${c_std}\n" \
	       "${msg_inuse}"
        return 0
    fi 
}
...
function check_samplerate() {
    ...
    while read -r aplayline; do
        ...
        LANG=C aplay "${aplay_args[@]}" 2>&1 <<< "${pseudo_silent_audio}" || \
...
$(declare -p BASH_REMATCH)"
            ...
            debug "${LINENO}" "alsa error: $?: \`$(declare -p aplayline)'"
            debug "${LINENO}" "using arguments: $(declare -p aplay_args)"
	fi
    done< <(
        LANG=C aplay "${aplay_args[@]}" 2>&1 <<< "${pseudo_silent_audio}"
    )
...
function get_uac_formats_rates() {
    ...
    playback_re="^Playback:"
    capture_re="^Capture:"
    inside_interface=
    format_found=
    declare -A uac_formats
    debug "${LINENO}" "iterate lines in streamfile \`${ALSA_PATHS_STREAMFILES[${idx}]}'"
    while read -r line; do
        if [[ "${line}" =~ ${capture_re} ]]; then
	    debug "${LINENO}" "end of playback interfaces"
	    break
        elif [[ "${line}" =~ ${playback_re} ]]; then
            debug "${LINENO}" "start of playback interfaces"
            continue
        fi
	## reset (previous) format_found
	if [[ "${line}" =~ ${interface_re} ]]; then
	    debug "${LINENO}" "new interface found ${BASH_REMATCH[1]}"
	    inside_interface=true
	    format_found=
	else
	    ## continuation of interface 
	    if [[ ${inside_interface} ]]; then 
...
function alsa_uac_endpoint() {
    ...
    ## based on 
    ## https://github.com/torvalds/linux/blob/master/sound/usb/proc.c
    ## Line 111: snd_iprintf(buffer, "    Endpoint: 0x%02x (%d %s) (%s)\n",
    ##   +1: fp->endpoint,
    ##   +2: fp->endpoint & USB_ENDPOINT_NUMBER_MASK,
    ##   +3: fp->endpoint & USB_DIR_IN ? "IN" : "OUT",
    ##   +4: sync_types[(fp->ep_attr & USB_ENDPOINT_SYNCTYPE) >> 2]);
    ## TODO: unsure which range this is; have seen 1, 3 and 5
    ##       or 0, 1, 2 or 3 according to ./sound/usb/card.h
    ##   +3: USB_DIR_IN: "IN|OUT",
    ##   +4: USB_ENDPOINT_SYNCTYPE: "NONE|ASYNC|ADAPTIVE|SYNC"
    streamfile="$1"
    endpoint_type=""
...
function apply_filter_custom() {
    ...
    oricount="${#FILTERED_INDICE[@]}"
    for idx in "${FILTERED_INDICE[@]}"; do
        if [[ "${ALSA_MATCHSTRINGS["${idx}"]}" =~ ${OPT_FILTER_CUSTOM} ]]; then
            debug "${LINENO}" "\`${ALSA_MATCHSTRINGS[${idx}]}' matches \`${OPT_FILTER_CUSTOM}'"
        else
...
function apply_filter_devtype() {
    ...
    oricount="${#FILTERED_INDICE[@]}"
    for idx in "${FILTERED_INDICE[@]}"; do
        if [[ "${ALSA_DEVTYPES["${idx}"]}" == "${OPT_FILTER_DEVTYPE}" ]]; then
            debug "${LINENO}" "\`${ALSA_DEVTYPES[${idx}]}' matches \`${OPT_FILTER_DEVTYPE}'"
        else
            # shellcheck disable=SC2184	    
            unset FILTERED_INDICE["${idx}"]
        fi
...
function alsa_carddev_arrays() {
    ...
    while read -r aplayline; do
        ...
        if [[ "${aplayline}" =~ ${ALSA_CARDLINE_RE} ]]; then
            ...
            alsa_cardname="${BASH_REMATCH[2]}"
            alsa_cardlabel="${BASH_REMATCH[3]}"
            devnr="${BASH_REMATCH[4]}"
            alsa_devname="${BASH_REMATCH[5]}"
            alsa_devlabel="${BASH_REMATCH[6]}"
            ## constructed
            matchstring="hw:${cardnr},${devnr}"
            matchstring+=" ${alsa_cardname} ${alsa_cardlabel}"
            matchstring+=" ${alsa_devname} ${alsa_devlabel}"
            ## set DEVTYPE
            streamfile="/proc/asound/card${cardnr}/stream${devnr}"
            if [[ -f "${streamfile}" ]]; then
                devtype="${DEVTYPE_UAC}"
                usbid_file="/proc/asound/card${cardnr}/usbid"
                if [[ -f "${usbid_file}" ]]; then                    
                    read -r uac_usbid < "${usbid_file}"
                    debug "${LINENO}" "uac_usbid \`${uac_usbid}' from \`${usbid_file}'"
                    # shellcheck disable=SC2207
                    manufacturer_re="^[[:space:]]*iManufacturer[[:space:]]*[0-9]+[[:space:]]*(.*)$"
                    product_re="^[[:space:]]*iProduct[[:space:]]*[0-9]+[[:space:]]*(.*)$"
                    while read -r line; do
                        if [[ "${line}" =~ ${manufacturer_re} ]]; then
                            uac_manufacturer="${BASH_REMATCH[1]}"
                        elif [[ "${line}" =~ ${product_re} ]]; then
                            uac_product="${BASH_REMATCH[1]}"
                            break
                        fi
                    done < <(lsusb -v -d "${uac_usbid}" 2>/dev/null) 
                    alsa_cardlabel="${uac_manufacturer} - ${uac_product}"
                else
                    die "no such usbid file \`${usbid_file}'."
                fi
            else
                streamfile="${MSG_PROP_NOTAPPLICABLE}"
            ...
            ALSA_LABELS_CARDS+=("${alsa_cardlabel}")
            ALSA_NAMES_CARDS+=("${alsa_cardname}")
            ALSA_IDS_DEVS+=("${devnr}")
            ALSA_NAMES_DEVS+=("${alsa_devname}")
            ALSA_LABELS_DEVS+=("${alsa_devlabel}")
...
MSG_ERROR_INUSE="in use by"
MSG_ERROR_PULSEAUDIO="${MSG_ERROR_INUSE} pulseaudio"
MSG_ERROR_PASUSPENDER="run \`pasuspender -- ${APP_NAME_AC}'"
